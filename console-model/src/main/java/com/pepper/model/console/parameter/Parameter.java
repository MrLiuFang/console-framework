...
public class Parameter extends BaseModel {
	...
	@Size(min=3,max=30,message="编码请输入{min}至{max}个字符",groups= {Insert.class})
	@Column(name = "code", nullable = false,length=30,unique=true)
	private String code;

	@NotBlank(message="请输入值",groups= {Insert.class,Update.class})
	@Size(max=30,message="值请输入不超过{max}个字符",groups= {Insert.class,Update.class})
	@Column(name = "value", nullable = false,length=30)
	private String value;

	@Size(max=500,message="备注不要超过{max}个字符",groups= {Insert.class,Update.class})
